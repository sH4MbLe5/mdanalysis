...
class Universe(object):
    ...
    **kwargs: extra arguments are passed to the topology parser.
    ...
    def from_ase(cls, atoms: 'ase.Atoms', elements_from_types: bool = False, guess_masses_from_types: bool = False) -> 'Universe':
        ...
        ----------
        atoms: ase.Atoms
            MDAnalysis.Universe object from ase.Atoms object

        Returns
        -------
        universe: Universe
        """
        u = cls.empty(n_atoms=len(atoms), n_residues = len(atoms), atom_resindex=[0]*len(atoms), trajectory=True)
        ...
        if 'resid' in atoms.arrays:
            ...
            u.add_TopologyAttr('resids', [0] * len(atoms))
